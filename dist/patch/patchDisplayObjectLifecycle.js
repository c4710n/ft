...
function _bindComponent(displayObject, component) {
  if (displayObject.added && component.added === false) {
    ...
    if (!FT_PRODUCTION_MODE) {
      ...
      console.log("[lifecycle] bind component ".concat(component.constructor.name, " for ").concat(displayObject.constructor.name));
    }
...
function _unbindComponent(displayObject, component) {
  ...
  if (component.added === true) {
    ...
    if (!FT_PRODUCTION_MODE) {
      // eslint-disable-next-line
      console.log("[lifecycle] unbind component ".concat(component.constructor.name, " for ").concat(displayObject.constructor.name));
    }
  }
}
...
function callPostAdd(container) {
  ...
  if (container.components) {
    Object.values(container.components).forEach(function (component) {
...
//# sourceMappingURL=patchDisplayObjectLifecycle.js.map