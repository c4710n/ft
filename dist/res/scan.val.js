...
module.exports = ({
  ...
  const resources = paths.map(($path, index) => {
    ...
    const name = $path.replace(addSep(_dir), '') // remove useless prefix
    .replace(sepRE, '.').replace(extname, '');
  ...
  const imports = resources.map(({
    module,
    path
  }) => `import ${module} from '${path}'`).join('\n');
  const objects = resources.map(({
    module,
    name,
    basename,
    type
  }) => `  {name: '${name}', basename: '${basename}', type: '${type}', url: ${module}}`).join(',\n');
  const code = `
${imports}
const objects = [
${objects}
...
//# sourceMappingURL=scan.val.js.map